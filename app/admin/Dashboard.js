...
const OPEN_STATUSES = ['pending_reply', 'awaiting_customer'];
const ARCHIVE_STATUSES = ['converted', 'closed'];

function EnquiryItem({ item, enquiryStatusPendingId, deletingEnquiryId, enquiryErrors, onStatusChange, onDelete }) {
    return (
        <li className="border border-gray-200 p-4 rounded-lg bg-gray-50 hover:bg-white transition">
...
export default function Dashboard({ galleryItems, whatsappNumber, logo, processStages, enquiries, testimonials, socialMediaLinks }) {
    ...
    const openEnquiries = enquiries.filter((e) => OPEN_STATUSES.includes(e.status));
    const archivedEnquiries = enquiries.filter((e) => ARCHIVE_STATUSES.includes(e.status));
            ...
            <div className="mx-auto max-w-3xl">
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 px-6 py-5 mb-10">
                    <div className="flex items-center justify-between">
                        ...
                        <form action={logout}>
                            <button type="submit" className="bg-gray-200 border border-gray-400 px-3 py-2 text-sm font-mono hover:bg-gray-300 cursor-pointer rounded px-6">
                                Log out
                            </button>
                        </form>
                    </div>
                ...
                {view === 'menu' && (
                    <section className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                        <div className="mb-5 pb-4 border-b border-gray-100">
                            <h2 className="text-lg font-bold text-gray-900">What do you want to do?</h2>
                            <p className="text-xs text-gray-500 mt-1 uppercase tracking-wide">Select an option</p>
                        </div>
                        <div className="space-y-3">
                            <button
                                type="button"
                                onClick={() => setView('enquiries')}
                                className="bg-gray-200 border border-gray-400 px-3 py-2 text-sm font-mono hover:bg-gray-300 cursor-pointer rounded w-full text-left"
                            >
                                Manage Enquiries
                                {openEnquiries.length > 0 && (
                                    <span className="ml-2 text-xs text-gray-600">({openEnquiries.length} open)</span>
                                )}
                            </button>
                            <button
                                type="button"
                                onClick={() => setView('site-details')}
                                className="bg-gray-200 border border-gray-400 px-3 py-2 text-sm font-mono hover:bg-gray-300 cursor-pointer rounded w-full text-left"
                            >
                                Update Site Details
                            </button>
                ...
                {view === 'enquiries' && (
                    <section className="mb-10 bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                        <div className="mb-5 pb-4 border-b border-gray-100">
                            <div className="flex items-center justify-between">
                                ...
                                <button
                                    ...
                                    className="bg-gray-200 border border-gray-400 px-3 py-2 text-sm font-mono hover:bg-gray-300 cursor-pointer rounded"
                                >
                                    Back
                                </button>
                            </div>
                        ...
                        <div className="flex gap-2 mb-5">
                            <button
                                type="button"
                                onClick={() => setEnquiryTab('open')}
                                className={`px-3 py-2 text-sm font-mono rounded border cursor-pointer ${
                                    enquiryTab === 'open'
                            ...
                            >
                                Archive ({archivedEnquiries.length})
                            </button>
                        </div>
                        ...
                        {enquiryTab === 'open' && (
                            <>
                                {openEnquiries.length === 0 ? (
                                    <p className="text-sm text-gray-500 py-4">No open enquiries.</p>
                                ) : (
                                    <ul className="space-y-3">
                                        {openEnquiries.map((item) => (
                                            <EnquiryItem
                                                key={item.id}
                                                item={item}
                                                enquiryStatusPendingId={enquiryStatusPendingId}
                                                deletingEnquiryId={deletingEnquiryId}
                        ...
                        {enquiryTab === 'archive' && (
                            <>
                                {archivedEnquiries.length === 0 ? (
                                    <p className="text-sm text-gray-500 py-4">No archived enquiries.</p>
                                ) : (
                                    <ul className="space-y-3">
                            ...
                            <div className="mb-5 pb-4 border-b border-gray-100">
                                <div className="flex items-center justify-between">
                                    <div>
                                        ...
                                        <p className="text-xs text-gray-500 mt-1 uppercase tracking-wide">Update site content</p>
                                    </div>
                                    <button
                                        type="button"
                                        onClick={() => setView('menu')}
